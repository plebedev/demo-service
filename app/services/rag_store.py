...
@dataclass(frozen=True)
class RagTextChunkInput:
    """One chunk ready for database-side embedding."""
...
class RagStore:
    ...
    def create_document_with_oracle_embeddings(
        self,
        session: Session,
        *,
        source: str,
        labels: list[str],
        chunks: list[RagTextChunkInput],
        oracle_model_name: str,
        title: str | None = None,
        content_sha256: str | None = None,
        metadata: dict[str, Any] | None = None,
    ) -> RagDocument:
        """Create a document and embed chunks with Oracle VECTOR_EMBEDDING."""
        if session.get_bind().dialect.name != "oracle":
            raise RuntimeError("Oracle database-side embeddings require Oracle.")
        ...
        model_name = self._oracle_model_name(oracle_model_name)
        for chunk in chunks:
            self._insert_oracle_embedded_chunk(
                session,
                document.id,
                chunk,
                oracle_model_name=model_name,
            )
    ...
    def search_chunks_with_oracle_query(
        self,
        session: Session,
        *,
        labels: list[str],
        ...
        """Search Oracle chunks by embedding the query inside the database."""
        if session.get_bind().dialect.name != "oracle":
            raise RuntimeError("Oracle database-side search requires Oracle.")
    ...
    def _insert_oracle_embedded_chunk(
        self,
        session: Session,
        document_id: int,
        chunk: RagTextChunkInput,
        *,
        oracle_model_name: str,
    ) -> None:
        params = {
        ...
        sql = text(
            ...
            VALUES (
                ...
                :chunk_text,
                :metadata_json,
                VECTOR_EMBEDDING({oracle_model_name} USING :chunk_text AS DATA)
            )
            """
        )
        session.execute(sql, params)
    ...
    def _validate_text_chunk(self, chunk: RagTextChunkInput) -> None:
        if not chunk.chunk_text.strip():
            raise ValueError("RAG chunk text cannot be empty.")
        if chunk.chunk_index < 0:
            raise ValueError("RAG chunk index cannot be negative.")
    ...
    def _oracle_model_name(self, model_name: str) -> str:
